...
class AttitudeControlEnv(gym.Env):
    ...
    def step(self, action):
        ...
        self.state = (self.q_error[3], self.q_error[0], self.q_error[1], self.q_error[2],  self.omega[0], self.omega[1], self.omega[2])
    ...
    def Reward2(self):
        ...
        if not done:
            ...
            if qs >= 0.99999762:
                if qs > self.qs_prev:
                    reward += -5*np.log(omega_magnitude/self.max_ang_velo)
                else:
                    reward += -2*np.log(omega_magnitude/self.max_ang_velo)

            if qs < 0.99999762 and self.qs_prev >= 0.99999762:
                reward += -1
